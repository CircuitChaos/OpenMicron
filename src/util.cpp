...
uint32_t util::crc32(uint32_t crc, const void *data, size_t size)
{
	// extracted from: http://web.archive.org/web/20080303102530/http://c.snippets.org/snip_lister.php?fname=crc_32.c

	static const uint32_t tbl[] = { /* CRC polynomial 0xedb88320 */
...
std::string util::toPrintable(const std::string &s)
{
	std::string rs;
	for (const auto &i: s)
	{
		if (i < 0x20 || i >= 0x7F)
			rs += util::format("<%02x>", i);
		else
			rs.push_back(i);
	}
	return rs;
