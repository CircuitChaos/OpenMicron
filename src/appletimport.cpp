...
/*
bool applet::CImport::parseNumber(const std::string &number, bool &isNeg, uint32_t &dec, uint32_t &frac)
{
	if (number.empty())
	{
		logError("empty number");
		return false;
	}

	// xxx: more checking is probably needed here

	std::string sdec, sfrac;
	const auto pos(number.find('.'));
	if (pos == std::string::npos)
		sdec = number;
	else
	{
		sdec = number.substr(0, pos);
		sfrac = number.substr(pos + 1);
	}

	dec = strtol(sdec.c_str(), NULL, 10);
	frac = strtol(sfrac.c_str(), NULL, 10);
	...
	logd("Separated number %s to: isNegative=%s, dec=%u, frac=%u", number.c_str(), isNeg ? "yes" : "no", dec, frac);
	return true;
}

bool applet::CImport::parseCtcss(const std::string &number, uint16_t &ctcss10)
{
	bool isNeg;
	uint16_t dec, frac;
	if (!splitNumber(number, isNeg, dec, frac))
		return false;

	if (isNeg)
	{
		logError("CTCSS %s can't be negative", number.c_str());
		return false;
	}

	if (frac > 9)
	{
		logError("too many digits in fractional part of CTCSS %s", frac, number.c_str());
		return false;
	}

	ctcss10 = dec * 10 + frac;

	// we allow special value 50.0 as original tool sometimes sets it in default CTCSS field
	if (ctcss10 == 500)
		return true;

	const std::string scts(util::format("%u.%u", dec, frac));
	// remember that last position in CTSTBL is 'def'
	for (size_t i(0); i < sizeof(CTSTBL) / sizeof(*CTSTBL) - 1; ++i)
		if (CTSTBL[i] == scts)
			return true;

	logError("CTCSS %s not found in CTCSS table", number.c_str());
	return false;
}
*/
